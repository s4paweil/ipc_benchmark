...
public class UDS extends IPCBase {
    @Override
    public void runBenchmark(int[] packetSizes, int iterations, long TOTAL_DATA_SIZE) throws IOException, InterruptedException {
        ...
        for (int packetSize : packetSizes) {
            ...
            System.out.println("Unix Domain Socket");
    ...
    protected void compileServerAndClient() throws IOException, InterruptedException {
        Process compileServer = Runtime.getRuntime().exec("javac -cp src src/main/java/org/betriebssysteme/uds/Server.java");
        compileServer.waitFor();
        compileServer.destroy();

        Process compileClient = Runtime.getRuntime().exec("javac -cp src src/main/java/org/betriebssysteme/uds/Client.java");
    ...
    private Process startClientProcess(int packetSize, long TOTAL_DATA_SIZE) throws IOException {
        return Runtime.getRuntime().exec("java -cp src/main/java org.betriebssysteme.uds.Client " + packetSize + " " + TOTAL_DATA_SIZE);
    }

    private Process startServerProcess() throws IOException {
        return Runtime.getRuntime().exec("java -cp src/main/java org.betriebssysteme.uds.Server");
    }
}